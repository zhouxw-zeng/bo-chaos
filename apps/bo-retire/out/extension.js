...
const utils_1 = require("@mono/utils");
const const_1 = require("@mono/const");
function completeToTargetDigits(number, digits = 2) {
  return String(number).padStart(digits, "0");
}
function getCountDown(tuixiu) {
  const now = (0, utils_1.dayjs)();
  const diff = tuixiu.diff(now);
  const duration = utils_1.dayjs.duration(diff);
  // 修改计算逻辑
...
const { boTuiXiuDay } = const_1.tuixiu;
